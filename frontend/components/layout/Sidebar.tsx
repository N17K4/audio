    ...
    <>
      <aside
        ...
        <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto overflow-x-hidden">
          ...
          {(Object.keys(TASK_LABELS) as TaskType[]).filter(t => t !== 'media' && t !== 'doc' && t !== 'misc').map(t => (
            <NavItem key={t} page={t} label={TASK_LABELS[t]} icon={<TaskIcon task={t} />} />
          ))}
        </nav>

        {/* 底部导航 - 格式转换 & 扩展功能 & 扩展功能2 & 任务列表 & 系统工具 */}
        <div className="border-t border-sky-100 dark:border-slate-800 px-2 py-2 space-y-0.5">
          ...
          <NavItem page="doc" label="文档工具" icon={<TaskIcon task="doc" />} />
          <NavItem page="misc" label="扩展功能" subtitle="图像·翻译·视频" icon={<TaskIcon task="misc" />} />
