...
def _patch_torchaudio_soundfile():
    """torchaudio 2.6+: torchcodec 未インストール時に soundfile フォールバック。"""
...
def handle_request(conn: socket.socket, components, device, engine_mod, last_active: list) -> None:
    ...
    while True:
        try:
            ...
            if idle >= IDLE_TIMEOUT:
                print(f"[seed_vc_worker] 空闲 {idle:.0f}s，自动退出", file=sys.stderr, flush=True)
                _cleanup()
            continue
        except OSError:
            break
        t = threading.Thread(target=handle_request, args=(conn, components, device, engine_mod, last_active), daemon=True)
        t.start()
