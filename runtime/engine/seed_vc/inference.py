...
# 仅在未由外部（build_engine_env）注入绝对路径时才设置，避免覆盖绝对路径
...
def crossfade(chunk1, chunk2, overlap):
    # 限制 actual_overlap 不超过两个 chunk 的实际长度，避免广播错误
    actual_overlap = min(overlap, len(chunk1), len(chunk2))
    if actual_overlap <= 0:
        return chunk2
    fade_out = np.cos(np.linspace(0, np.pi / 2, actual_overlap)) ** 2
    fade_in = np.cos(np.linspace(np.pi / 2, 0, actual_overlap)) ** 2
    chunk2[:actual_overlap] = chunk2[:actual_overlap] * fade_in + chunk1[-actual_overlap:] * fade_out
    return chunk2
...
def main(args):
    ...
    if f0_condition:
        ...
        F0_ori = torch.from_numpy(F0_ori.astype('float32')).to(device)[None]
        F0_alt = torch.from_numpy(F0_alt.astype('float32')).to(device)[None]
    ...
    while processed_frames < cond.size(1):
        ...
        # MPS 不支持 autocast，回退到 cpu；cuda/cpu 正常使用 autocast
        _autocast_device = device.type if device.type in ("cuda", "cpu") else "cpu"
        with torch.autocast(device_type=_autocast_device, dtype=torch.float16 if fp16 else torch.float32):
            # Voice Conversion
            ...
            # BigVGAN conv_pre kernel_size=7，输入太短会崩溃；对齐到最小帧数
            if vc_target.size(-1) < 7:
                vc_target = torch.nn.functional.pad(vc_target, (0, 7 - vc_target.size(-1)))
