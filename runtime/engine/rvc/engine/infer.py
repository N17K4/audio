...
def main() -> int:
    ...
    # ── 环境诊断（import 前打印，方便定位缺包问题）─────────────────────────────
    print(f"[rvc] Python {sys.version}", file=sys.stderr, flush=True)
    print(f"[rvc] sys.executable={sys.executable}", file=sys.stderr, flush=True)
    print(f"[rvc] PYTHONPATH={os.environ.get('PYTHONPATH', '(未设置)')}", file=sys.stderr, flush=True)
    print(f"[rvc] sys.path={sys.path}", file=sys.stderr, flush=True)

    # 检查关键依赖包是否可用，逐一报告
    _deps = ["torch", "torchaudio", "numpy", "scipy", "soundfile",
             "librosa", "faiss", "praat", "pyworld", "torchcrepe", "rvc_python"]
    for _dep in _deps:
        _mod_name = "faiss" if _dep == "faiss" else ("parselmouth" if _dep == "praat" else _dep)
        try:
            _m = __import__(_mod_name)
            _ver = getattr(_m, "__version__", "?")
            print(f"[rvc]   ✓ {_dep} {_ver}", file=sys.stderr, flush=True)
        except ImportError as _ie:
            print(f"[rvc]   ✗ {_dep} 缺失: {_ie}", file=sys.stderr, flush=True)

    try:
        from rvc_python.infer import RVCInference
    except ImportError as _e:
        import traceback as _tb
        print(f"[rvc] rvc_python 导入失败: {_e}", file=sys.stderr)
